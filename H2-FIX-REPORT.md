...
- **لا** مصادقة لكل مستخدم ولا تفويض أدوار على الخادم؛ كل `/api/*` خلف مفتاح API ثابت مشترك (`[REDACTED]`).
...
- **المفتاح الافتراضي** `[REDACTED]` ما زال مرجعاً احتياطياً للعميل وتشغيلاً افتراضياً للخادم في غياب `.env`. النطاق محدود الآن بالجهاز (POS) بفضل التفويض، لكن يُنصح بتعيين `API_KEY` قوي في البيئة (ضمن W1/W4 لاحقاً).
