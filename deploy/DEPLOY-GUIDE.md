...
## التحديث الجديد: التشغيل السحابي واللايف

النسخة الحالية تجعل الوضع السحابي هو الافتراضي، وتستخدم قناة Supabase Realtime لتحديث الطلبات والطاولات ولوحة الإدارة على كل الأجهزة. بعد تنفيذ المخطط وملف RPC شغّل أيضًا:

```text
deploy/supabase-live.sql
```

من Supabase SQL Editor مرة واحدة. هذا الملف يضيف الجداول التشغيلية إلى `supabase_realtime` ويضبط `REPLICA IDENTITY FULL` لتصل أحداث التعديل والحذف إلى الشاشات المتصلة. لا تستخدم `service_role` داخل الواجهة.

للتشغيل المحلي المؤقت فقط يمكن ضبط:

```js
localStorage.setItem('luccaDataMode', 'local');
```

أما التشغيل الطبيعي فيترك القيمة `supabase` أو يحذفها، لأن التطبيق يختار السحابي افتراضيًا. يجب نشر `cloud-config.js` مع الواجهة والتأكد من أن رابط المشروع و`anon` key يعودان إلى مشروع Supabase الصحيح.
>
