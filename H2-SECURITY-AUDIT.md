...
كما أن `API_KEY` الافتراضي على الخادم **`[REDACTED]`** مثبّت في الكود (index.ts:10) وكمرجع احتياطي للعميل (`'[REDACTED]'`).
...
2. **المفتاح الافتراضي معروف في الكود المصدري** (`[REDACTED]`) ومرجع احتياطي عبر كل ملفات العميل → في غياب `.env` يتم تشغيل الخادم بالمفتاح الافتراضي فعلياً.
