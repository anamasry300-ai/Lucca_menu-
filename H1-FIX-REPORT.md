...
النظام استخدم **مفتاح API ثابتاً مشتركاً** (`[REDACTED]`) لحماية كل `/api/*`، وكان:
- **حرفياً** في الواجهة (`'[REDACTED]'` مرجع احتياطي في `index.html` و `admin/database.js`).
...
| المصدر | النوع |
|---|---|
| `index.html:2970,4839` | `localStorage.luccaApiKey \|\| '[REDACTED]'` (حرفي) |
| `admin/database.js:2552,2604,2719` | `localStorage.luccaApiKey \|\| '[REDACTED]'` (حرفي) |
...
| الخادم | `API_KEY` default حرفي `[REDACTED]` (index.ts, auth.ts) |
...
- **`admin/database.js`:** أزال كل `'[REDACTED]'` الحرفية (2552/2604/2719 → `''`) وحذف جلب `public-key` الميت في `initSystem`.
...
- `API_KEY` الافتراضي الحرفي `[REDACTED]` بقي **في الخادم فقط** (env fallback لنافذة الهجرة) — ليس في الواجهة.
